    ...
    <template v-else>
      <div class="project-list">
        <div v-for="p in pagedProjects" :key="p.link || projectFor(p).title" class="project-card">
          ...
          <p class="card-desc">{{ projectFor(p).description }}</p>
          <div class="card-tags">
            <el-tag v-for="tag in projectFor(p).tags" :key="tag" size="small" effect="light">{{ tag }}</el-tag>
          </div>
        </div>
      </div>

      <div class="project-pagination">
        <el-pagination background layout="prev, pager, next" :total="projects.length" :page-size="pageSize"
...
const projectFor = (p) => p[locale.value] || p.zh || {}
const projects = ref([])
